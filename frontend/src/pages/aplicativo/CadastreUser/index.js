...
const validationSchema2 = yup.object().shape({
  ...
  responsible: yup
    .string()
    .min(7, "Digite o nome completo do responsável")
...
function CadastreUser(props) {
  const [type, setType] = useState("Associado");
  ...
  const setDates = (values) => {
    if (image === null) {
      Alert.alert(
        "Foto obrigatória!",
        "Para efetuar o cadastro, uma foto de perfil deve ser selecionada!"
      );
    } else {
  ...
  useEffect(() => {
    Alert.alert("Cadastro", "Todos os campos devem ser preenchidos!", [
      { text: "Ok" },
    ]);
  }, []);
          ...
          {type === "Associado" ? (
            <Formik
              initialValues={{
                ...
                registration_number: "",
                masp: "",
                blood_type: "Não declarado",
                marital_status: "Não declarado",
                rg: "",
                cpf: "",
                date_of_birth: "",
                naturalness: "",
